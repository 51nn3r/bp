...
class LoRA_SVD_Zero(BaseLoRA):
    ...
    def __init__(
        ...
        self._alpha = nn.Parameter(torch.zeros(1, dtype=dtype, device=device))
        ...
        with torch.no_grad():
            for m in self._svd_matrices:
                if m.dim() == 2:
                    ...
                    core_v = (diag_sqrt @ v_r).to(self._dtype)
    ...
    def enable_grad(self):
        if len(self._shape) == 2:
            ...
            self._svd_matrices[0].requires_grad = True
